...
extract($_POST);
if(isset($_POST['role'])){
    $query = "INSERT INTO rolemaster(role) VALUES('$role')";
    $jobQuery = mysqli_query($conn,$query);
if($jobQuery==1)
    {
        $last_id = mysqli_insert_id($conn);
        $s = strval($last_id);
        $sql = "SELECT * FROM rolemaster WHERE roleId = $s";
    ...
    $response = array('Message'=>"All Role  Data Inserted Successfully","Data"=>$records ,'Responsecode'=>200);
		}else{
